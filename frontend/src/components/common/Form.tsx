...
interface FormProps {
  ...
  title: string;
  actions?: { name: string; action: () => void; icon?: React.ReactNode }[];
}

export default function Form({ children, title, actions }: FormProps) {
  return (
    <form className="w-full bg-misc-white p-6 rounded-md flex flex-col justify-between items-center gap-4 shadow-custom">
      ...
      <div className="w-full flex flex-col-reverse md:flex-row-reverse justify-start items-center gap-4">
        {actions?.map((_action, index) => (
          <button
            ...
            } px-8 py-2 rounded-md flex items-center justify-between gap-2`}
          >
            {_action?.icon ?? null}
            {_action?.name ?? ''}
          </button>
